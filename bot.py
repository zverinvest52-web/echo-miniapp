...
def create_task(user_id: int, title: str, description: str = None, priority: int = 5, deadline: str = None) -> dict:
    """Создать задачу"""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()

    c.execute('''INSERT INTO tasks (user_id, title, description, priority, deadline)
        VALUES (?, ?, ?, ?, ?)''',
    ...
    return {"id": task_id, "title": title, "status": "active"}
...
Я помогу тебе управлять задачами с помощью голосового ввода и AI.

🚀 *Возможности:*
• Быстрое добавление задач
• Шаблоны задач (митинг, код-ревью и т.д.)
• Отслеживание приоритетов
• Статистика выполнения

📱 *Как использовать:*
1. Нажми "📋 Открыть Echo"
2. Добавь задачу через Mini App
3. Или просто напиши мне задачу!
...
@app.get("/")
async def root():
    return {"status": "running", "service": "Echo Bot + API", "version": "2.0.0"}
...
async def create_task_api(user_id: int, task: TaskCreate):
    result = create_task(user_id, task.title, task.description, task.priority, task.deadline)
    return result
...
if __name__ == "__main__":
    ...
    logger.info("🚀 Echo Bot запускается...")
